...
| Optional editor  | emacs                | pacman (extra)          | **opt-in** `[y/N]` prompt in 00-base.sh (skipped by default); pgtk/Wayland build; ships `config/emacs/init.el` (single-file, no packages, pywal-driven) |
| Browser          | brave                | **AUR — brave-bin**     | default; xdg-mime default for http(s)/ftp/html |
| Media player     | vlc                  | pacman (extra)          | default for video/audio MIME types; ships `config/vlc/vlcrc` — snapshots go to `~/Pictures/vlc-snapshots` (sequential names), `@HOME@` placeholder expanded by 30-dotfiles.sh; decoding left on VLC's auto default (see file comments) |
...
#    (deny incoming / allow outgoing). Asks one [y/N] question:
#    whether to also install emacs (official extra repo) — default no.
./scripts/00-base.sh
...
#    the MIME associations defined in hyprland.conf, expands the
#    @HOME@ placeholder in vlc/vlcrc + pre-creates the VLC snapshot
#    dir, and drops the stray config/emacs/ copy if emacs wasn't
#    installed in step 1.
...
## Code editor setup (Zed, Neovim, optional Emacs, Ghostty)
...
**Emacs** is optional. `00-base.sh` asks one `[y/N]` question (default
no); if you opt in it installs the official `extra` package (the pgtk
build, native Wayland). Its config ships at `config/emacs/init.el` —
single-file, zero packages, pywal-driven colors (loads
`~/.cache/wal/colors.el`, generated by the custom pywal template
`config/wal/templates/colors.el`, with a baked Catppuccin Mocha
fallback), plus sane defaults (no backup files, electric-pair, relative
line numbers) and `C-c`-prefixed binds (`C-c w` save, `C-c q` kill
buffer, `C-c e` dired-jump, `C-c b` switch buffer, `C-c n` toggle line
numbers). Note the XDG gotcha documented in the file header: Emacs only
reads `~/.config/emacs/init.el` while `~/.emacs.d` does not exist.
`30-dotfiles.sh` removes the stray `~/.config/emacs/` copy if emacs
isn't installed, so opting in later is: `sudo pacman -S emacs`, re-run
`30-dotfiles.sh`.
...
└── config/
    ...
    ├── vlc/
    │   └── vlcrc                           snapshots -> ~/Pictures/vlc-snapshots; @HOME@ expanded by 30-dotfiles.sh
    ├── emacs/
    │   └── init.el                         optional (opt-in); single-file, no packages, pywal-driven
    ├── wal/
    │   └── templates/
    │       ├── colors-rofi.rasi            custom pywal template -> ~/.cache/wal/colors-rofi.rasi
    │       └── colors.el                   custom pywal template -> ~/.cache/wal/colors.el (read by emacs/init.el)
    └── applications/
